...
//const center1 = document.getElementById('center-div1')
//const center2 = document.getElementById('center-div2')
//const center3 = document.getElementById('center-div3')
const livesCount = document.querySelector('#livec')
canvas.width = 1024
canvas.height = 576
  ...
  for (let i = projectiles.length - 1; i >= 0; i--) {
    ...
    for (let j = powerUps.length - 1; j >= 0; j--) {
      ...
      if (
        ...
        projectiles.splice(i, 1)
        powerUps.splice(j, 1)
        player.powerUp = 'DoubleMachineGun'
...
//
  ...
  grids.forEach((grid, gridIndex) => {
    ...
    for (let i = grid.invaders.length - 1; i >= 0; i--) {
      ...
      for (let j = bombs.length - 1; j >= 0; j--) {
        ...
        //
        if (( Math.hypot( player.position.x - bomb.position.x, player.position.y - bomb.position.y ) < playerRadius + bomb.radius && bomb.active )&&
        !game.over ){
            health.value-=100
            endGame()
            
          }
          if (( Math.hypot( player.position.x - bomb.position.x, player.position.y - bomb.position.y ) < playerRadius + bomb.radius  )&&
        !game.over ){
            health.value-=100
            endGame()
  ...
  if (keys.space.pressed && player.powerUp === 'DoubleMachineGun' && frames % 2 === 0){
    projectiles.push(
      new Projectile({
        position: {
...
addEventListener('keydown', ({ key }) => {
  ...
  switch (key) {
    ...
    case ' ':
      ...
      if (player.powerUp === 'DoubleMachineGun') return
    ...
    case 'p':
      const g = GAMESTATE.PAUSED;
      keys.p.pressed = true
      togglePause()  
      break

    case 'r':
      const n = GAMESTATE.RUNNING
      keys.r.pressef = true
      toggleResume()
      break
